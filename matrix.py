...
class Matrix:
	...
	def __getitem__(self, indexes):
		...
		return self.data[indexes[0] * self.height + indexes[1]]
	...
	def __setitem__(self, indexes, value):
		...
		if indexes[0] >= self.height or indexes[1] >= self.width:
			raise IndexError()
		# TODO: L'affectation
		self.data[indexes[0] * self.height + indexes[1]] = value
	...
	def __str__(self):
		...
		return format(self, "") # Bein oui, aussi simple que ça.
	...
	def __repr__(self):
		...
		return f"Matrix({self.height}, {self.width}, {self.data.__repr__()})" # Mieux que juste self.data pour le dernier, plus reliable
	...
	# TODO: Addition
	def __add__(self, other): # other est l'opérande de droite
	...
	def __sub__(self, other):
		return self + -other # On a défini la négation plus tôt.
	...
	def __mul__(self, other):
		if isinstance(other, Matrix):
			...
			return Matrix(self.height, self.width, [other * i for i in self.data])
		else:
			raise TypeError()
	...
	# # Défini implicitement, pas nécessaire.
